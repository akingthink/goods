...
class Baseinfo:
    ...
    def update_goods_last_spider_logs(self, source, goods_count, newest_url):
        ...
        goods_last_spider_logs.update(
            {'source': source}, {'source': source, 'goods_count': goods_count, 'newest_url': newest_url, 'spider_time': datetime.datetime.utcnow()}, True)
    ...
    def update_goods_count(self, source, goods_count):
        ...
        prev_goods_count = prev_goods_spider_logs[
            'goods_count'] if prev_goods_spider_logs is not None else 0
        total_count = int(prev_goods_count) + int(goods_count)
        goods_spider_log = {'source': source, 'goods_count': goods_count,
                            'total_count': total_count, 'created_time': datetime.datetime.utcnow()}
